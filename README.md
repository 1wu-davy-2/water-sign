...
- **8 个教学模块**，36 个小节 —— 5 个三座通用的心法模块 + 3 个星座专属模块（巨蟹 / 天蝎 / 双鱼）
- **63 条可复制话术**，按场景 + 星座双重筛选，支持一键复制与本地收藏
...
在 `Module` 上加 `sign: 'cancer' | 'scorpio' | 'pisces'`，首页就会把它归到
「按星座深入」分组，模块页内联的话术也会自动只显示**该星座适用**的（含通用话术），
不会把天蝎的卡片混进巨蟹页面。
...
话术库支持 `?sign=cancer` 预选星座（模块页的「还有 N 条 →」就是带着这个参数跳过去的）。
当前分布：`all` 24 条，巨蟹 18 条，天蝎 15 条，双鱼 21 条（可多标签重叠）。
